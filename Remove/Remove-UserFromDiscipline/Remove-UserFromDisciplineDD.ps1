...
$listItems = Get-PnPListItem -List $listName -PageSize 5000 | ForEach-Object {
    [PSCustomObject]@{
        ID           = $_["ID"]
        Name         = $_["Title"]
        CCRecipients = $_["CCRecipients"]
    }
}
Write-Log "Caricamento completato."

# Filtro elementi della lista contenenti la mail rimuovere la user mail.
foreach ($iten in $listItems) {
    $userDisciplines = $listItems | Where-Object { $_.CCRecipients -contains $UserEmailToRemove }
    Write-Host "$($userDisciplines)"
}
# Controllo presenza
