/*
    ...
    0
        - Set minimum width and height for the main buttons
            - Test here: https://onmsys.sharepoint.com/sites/Tecnimont-AMSDD/Documenti%20condivisi/Forms/AllItems.aspx
        - Set refresh and ext-settings buttons width and height to be equal to the main buttons

    1
        - exlcude publiccdn.sharepointonline.com from the search
...
async function GetSharePointSiteObjects(SPOMainSiteUrl) {
    try {
        ...
        // Await the promise for the tempListenerPromise to complete
        ...
        ButtonIds.forEach(SiteButtonId => {
            ...
            switch (SiteButtonId) {
                ...
                case "RecycleBinButton":
                    url = `${SPOMainSiteUrl}/_layouts/15/AdminRecycleBin.aspx?view=5`; //_layouts/15/RecycleBin.aspx?view=5
                    break;
...
async function GetPowerAutomateSiteObjects(environmentId, tabId) {
    try {
        ...
        let Flows_FilteredData = null;
        let SolutionsFlows_FilteredData = null;
        let LocalStorage_PA_Session_Token = null;
        let LocalStorage_Dynamics_Session_Token = null;
        const FlowsContainer = document.getElementById('FlowsContainer');
        const FlowsButton = document.getElementById('FlowsButton');
        const ShouldBustCache = localStorage.getItem('ShouldBustCache');


        // Temporary listener for the 'Send_Tokens_To_Popup' action
        const tempListenerPromise = new Promise(async (resolve, reject) => {
            const tempListener = async (request, sender, sendResponse) => {
                if (request.action === 'Send_Tokens_To_Popup') {
                    LocalStorage_PA_Session_Token = request.tokens.PA_Session_Object.secret;
                    LocalStorage_Dynamics_Session_Token = request.tokens.Dynamics_Session_Object.secret;
                    //LogToServiceWorker('Received Power Automate Token: ' + LocalStorage_PA_Session_Token);
                    //LogToServiceWorker('Received Dynamics Token: ' + LocalStorage_Dynamics_Session_Token);
                    //LogToServiceWorker('Dynamics InstanceApiUrl: ' + request.tokens.Dynamics_InstanceApiUrl);

                    chrome.runtime.onMessage.removeListener(tempListener);  // Remove listener

                    // Check if the token is null
                    if (LocalStorage_PA_Session_Token && LocalStorage_Dynamics_Session_Token) {

                        // Retrieve all Flows which are not in a solution
                        const Flows_API_Url = `https://api.flow.microsoft.com/providers/Microsoft.ProcessSimple/environments/${environmentId}/flows?api-version=2016-11-01&$filter=*&$include=*`;
                        if (ShouldBustCache === 'true') {
                            LogToServiceWorker('Cache buster requested.');
                            const CacheBuster = new Date().getTime();
                            Flows_API_Url = `${Flows_API_Url}&?cacheBuster=${CacheBuster}`;
                            localStorage.setItem('ShouldBustCache', 'false');
                        }
                        const Flows_API_Call = await fetch(Flows_API_Url, {
                            method: 'GET',
                            headers: {
                        ...
                        const Flows_Response = await Flows_API_Call.json();
                        //LogToServiceWorker('Retrieved flows: ' + JSON.stringify(Flows_Response));

                        // throw an error if no data is returned from the fetch
                        if (Flows_Response.error) {
                            return;
                        }

                        // Create a new object to store the desired properties
                        Flows_FilteredData = Flows_Response.value.map(item => ({
                            FlowDisplayName: item.properties.displayName,
                            FlowID: item.name
                        }));

                        // ! more org id to be retrieved, check
                        // Retrieve all Flows which are inside a solution
                        const SolutionsFlows_API_Url = `${request.tokens.Dynamics_InstanceApiUrl}/api/data/v9.2/msdyn_solutioncomponentsummaries?api-version=9.1&$filter=msdyn_componentlogicalname eq 'workflow'`;
                        if (ShouldBustCache === 'true') {
                            LogToServiceWorker('Cache buster requested.');
                            const CacheBuster = new Date().getTime();
                            SolutionsFlows_API_Url = `${SolutionsFlows_API_Url}&?cacheBuster=${CacheBuster}`;
                            localStorage.setItem('ShouldBustCache', 'false');
                        }
                        const SolutionsFlows_API_Call = await fetch(SolutionsFlows_API_Url, {
                            method: 'GET',
                            headers: {
                                'Authorization': `Bearer ${LocalStorage_Dynamics_Session_Token}`,
                            }
                        });
                        const SolutionsFlows_Response = await SolutionsFlows_API_Call.json();
                        //LogToServiceWorker('Retrieved solutions flows: ' + JSON.stringify(SolutionsFlows_Response));

                        // Throw an error if no data is returned from the fetch
                        if (SolutionsFlows_Response.error) {
                            return;
                        }

                        // Create a new object to store the desired properties
                        SolutionsFlows_FilteredData = SolutionsFlows_Response.value.map(item => ({
                            FlowDisplayName: item.msdyn_displayname,
                            FlowID: item.msdyn_workflowidunique
                        }));

                        // Merge arrays of both Flows and SolutionsFlows
                        filteredData = Flows_FilteredData.concat(SolutionsFlows_FilteredData);

                        // Sort the filteredData array by FlowDisplayName
                        filteredData.sort((a, b) => a.FlowDisplayName.localeCompare(b.FlowDisplayName));
                        //LogToServiceWorker(filteredData);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async tabs => {
        ...
        try {
            ...
            //let ErrorRow = error.stack.split('\n').slice(1).find(line => line.includes('at chrome-extension')).match(/:(\d+):\d+/)[1]; //"!include"->"include" //! Old, sometimes fails
            let ErrorRow = error.stack.split('\n').slice(1).find(line => line.includes('at')).match(/:(\d+):\d+/)[1];
            //let ErrorSource = error.stack.split('\n').slice(1).find(line => line.includes('at chrome-extension')).match(/([^\/]*\.js:\d+:\d+)/)[1]; //"!include"->"include" //! Old, sometimes fails
            let ErrorSource = error.stack.split('\n').slice(1).find(line => line.includes('at ')).match(/([^\/]*\.js:\d+:\d+)/)[1];
